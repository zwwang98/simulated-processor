...
void lockAttempted(const char* lockId, Thread* thread) {
    ...
    Thread* holdingThread = (Thread*) GET_FROM_MAP(const char *, lockToHoldingThread, lockId);
    if (!holdingThread) {
        sprintf(line, "[lockAttempted] thread %s is attempting lock %s and the lock is not held by any thread\n", thread->name, lockId);
        verboseLog(line);
    } else {
        if (holdingThread->priority < thread->priority) {
            sprintf(line, "[lockAttempted] thread %s is attempting lock %s and it need to donate priority to thread %s\n", thread->name, lockId, holdingThread->name);
            verboseLog(line);
            holdingThread->priority = thread->priority;
            sprintf(
                line,
                "[lockAttempted] thread %s just donated priority to thread %s. Now thread %s's priority is %d, thread %s's priority is %d\n",
                thread->name,
                holdingThread->name,
                thread->name,
                thread->priority,
                holdingThread->name,
                holdingThread->priority
            );
            verboseLog(line);
            bool isHoldingThreadBlocked = MAP_CONTAINS(Thread*, threadToWaitingLock, holdingThread);
            if (isHoldingThreadBlocked) {
                sprintf(
                    line,
                    "[lockAttempted] holdingThread is blocked, priority donation is required\n"
                );
                verboseLog(line);
                const char * blockHoldingThreadLock = (const char *) GET_FROM_MAP(Thread*, threadToWaitingLock, holdingThread);
                Thread* blockHoldingThreadThread = (Thread*) GET_FROM_MAP(const char *, lockToHoldingThread, blockHoldingThreadLock);
                blockHoldingThreadThread->priority = thread->priority;
                sprintf(
                    line,
                    "[lockAttempted] nested priority donatoin - thread %s is attempting lock %s and it need to donate priority to thread %s\n",
                    holdingThread->name,
                    blockHoldingThreadLock,
                    blockHoldingThreadThread->name
                );
                verboseLog(line);
                sprintf(
                    line,
                    "[lockAttempted] nested priority donatoin - thread %s just donated priority to thread %s. Now thread %s's priority is %d, thread %s's priority is %d\n",
                    holdingThread->name,
                    blockHoldingThreadThread->name,
                    holdingThread->name,
                    holdingThread->priority,
